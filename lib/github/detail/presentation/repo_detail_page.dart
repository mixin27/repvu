...
import 'package:repvu/github/core/presentation/no_result_display.dart';
  ...
  Widget build(BuildContext context) {
    return Consumer(
      builder: (context, watch, child) {
        final state = watch(repoDetailNotifierProvider);

        // FIXME: Unhandled Exception: setState() or markNeedsBuild() called during build.
        // state.maybeWhen(
        //   orElse: () {},
                  ...
                  loadSuccess: (state) {
                    return IconButton(
                      ...
                      onPressed: !state.repoDetail.isFresh
                          ? null
                          : () {
                              watch(repoDetailNotifierProvider.notifier)
                                  .switchStarredStatus(
              ...
              loadSuccess: (_) {
                if (_.repoDetail.entity == null) {
                  return const NoResultDisplay(
                    message:
                        'There are approximately all the details we could find about this repo right now.',
                  );
                } else {
                  return WebView(
